...
public class NamedPropertyRowMapper<T extends BaseDO<ID>, ID> implements RowMapper<T> {
    ...
    public T mapRow(ResultSet rs, int rowNumber) throws SQLException {
        ...
        Object ct = rs.getObject("create_time");
        if (ct instanceof Integer) {
            mappedObject.setCreateTime(new EzDate((Integer) ct));
        } else if (ct instanceof Long) {
            mappedObject.setCreateTime(new EzDate((Long) ct));
        } else if (ct instanceof String) {
            mappedObject.setCreateTime(new EzDate((String) ct));
        } else if (ct instanceof LocalDateTime) {
            mappedObject.setCreateTime(new EzDate((LocalDateTime) ct));
        }

        Object ut = rs.getObject("update_time");
        if (ut instanceof Integer) {
            mappedObject.setUpdateTime(new EzDate((Integer) ut));
        } else if (ut instanceof Long) {
            mappedObject.setUpdateTime(new EzDate((Long) ut));
        } else if (ut instanceof String) {
            mappedObject.setUpdateTime(new EzDate((String) ut));
        } else if (ut instanceof LocalDateTime) {
            mappedObject.setUpdateTime(new EzDate((LocalDateTime) ut));
        }
        try{
